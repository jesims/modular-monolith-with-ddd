...
public class MeetingFeesTests : TestBase
{
    ...
    public async Task Create_Then_Pay_MeetingFee_Test()
    {
        ...
        var meetingFees =
            await GetEventually(new GetMeetingFeesProbe(PaymentsModule, meetingId, x => x != null && x.Count > 0),
                5000);

        Assert.That(meetingFees[0].Status, Is.EqualTo(MeetingFeeStatus.WaitingForPayment.Code));

        var meetingFeePaymentId =
            await PaymentsModule.ExecuteCommandAsync(new CreateMeetingFeePaymentCommand(meetingFeeId));

        await PaymentsModule.ExecuteCommandAsync(new MarkMeetingFeePaymentAsPaidCommand(meetingFeePaymentId));

        meetingFees =
            await GetEventually(
                new GetMeetingFeesProbe(PaymentsModule, meetingId,
                    x => x.Any(y => y.Status == MeetingFeePaymentStatus.Paid.Code)), 10000);
